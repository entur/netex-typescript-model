...
- `scripts/ts-gen.ts` — end-to-end validation that assembled codegen output type-checks. Takes target entity names as positional CLI args and optional `--dest-dir` (default `/tmp`). Assembles the main interface + transitive deps (via `collectDependencyTree` + `generateInterface`) mirroring the schema viewer's Copy button, writes to `<dest-dir>/<Type>.ts`, and runs `tsc --noEmit --strict --skipLibCheck`. Validates that the codegen pipeline produces self-contained, type-safe TypeScript

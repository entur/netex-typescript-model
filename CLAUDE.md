...
- `scripts/lib/__tests__/` — per-module unit tests (mock schemas) and integration tests (real generated schema). Shared `test-helpers.ts` re-exports `loadNetexLibrary()` from `loader.ts`. `valid-roundtrip.test.ts` validates fake→XML output against NeTEx XSD via `xmllint`

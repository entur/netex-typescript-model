...
Convenience wrappers that shell out to `html-ts-gen/scripts/ts-gen.ts`:

- `gen-samples/gen-vehicle.sh` — generates `Vehicle.ts` with `--collapse-refs --collapse-collections`
- `gen-samples/gen-vehicletype.sh` — generates VehicleType `.ts` files with the hathor-specific `--exclude` list
- `gen-samples/gen-deckplan.sh` — generates DeckPlan `.ts` files with the NeTEx-Deckplan-Editor `--exclude` list
...
- `scripts/lib/classify.ts` — schema classification and role detection. Exports: `classifySchema` (discriminated `SchemaShape` union), `resolveType`, `isRefType`, `refTarget`, `isDynNocRef`, `unwrapMixed`, `defRole` (returns base role, stripping `/deprecated` suffix), `isDeprecated` (checks `/deprecated` suffix on `x-netex-role`), `countRoles`, `presentRoles`, `ROLE_DISPLAY_ORDER`, `ROLE_LABELS`
- `scripts/lib/schema-nav.ts` — inheritance walking and property flattening. Exports: `flattenAllOf`, `collectRequired`, `buildInheritanceChain`, `buildExclSet` (builds a `Set<string>` of property names to exclude, merging explicit exclusions with optional omnipresent filtering), `OMNIPRESENT_DEFS`, `ESSENTIAL_OMNI_PROPS`
...
- `scripts/lib/collapse.ts` — collapse resolution for `--collapse-refs` and `--collapse-collections`. Exports: `CollapseOpts`, `CollapsedRef`, `CollapsedColl`, `collapseRef` (ref property → `Ref<'Entity'>` / `SimpleRef`; accepts both `"reference"`-role defs and abstract-role defs whose name ends with `Ref`, so abstract ref heads also collapse), `collapseColl` (single-child RelStructure → child entity type), `resolveCollVerStruct` (def-name variant for BFS), `buildTypeOverrides` (builds `Map<canonName, typeStr>` for `generateInterface`), `REF_PREAMBLE`
- `scripts/lib/dep-graph.ts` — reverse index, dependency tree, ref-entity resolution. Exports: `buildReverseIndex`, `findTransitiveEntityUsers`, `resolveRefEntity`, `collectRefProps`, `collectExtraProps`, `collectDependencyTree` (BFS walker collecting transitive type deps as `DepTreeNode[]`, accepts optional `RemapTarget` callback for collapse), `resolveAlias`, `resolveConcreteElement` (abstract element head → first concrete substitution-group member by walking `x-netex-sg-members`; role-guarded, returns input unchanged if the starting def isn't abstract)
- `scripts/lib/data-faker.ts` — fake data generation and XML serialization. Primary export: `fake`. Also: `flattenFake` (transforms `fake()` output to generated-interface shape: unwraps collections, strips excluded props), `defaultForType`, `buildXml`, `toXmlShape` (schema-aware transform to XMLBuilder shape), `serialize` (composes `toXmlShape` + `buildXml`). Respects `x-netex-choice` annotations — only the first alternative from each choice group is emitted
- `scripts/lib/to-xml-shape.ts` — static generator for stem→XML projection functions and inline code blocks. Emits runtime helper functions (`strVal`, `attr`, `elem`, `child`, `mapArr`, `wrapArr`, `text`) plus per-entity functions that use them; deduplicates identical child functions into const aliases. `makeInlineCodeBlock` exports the main `xxxToXmlShape` function (placed first in output) while `reshapeComplex` dispatch and child functions remain unexported below it. Exports: `makeInlinedToXmlShape`, `makeInlineCodeBlock`, `emitHelpers` — all three accept `typed?: boolean` to emit TypeScript type annotations. When collapse is on, the emitted `refAttr(obj, key, xk?)` helper accepts an optional third argument that overrides the XML element name — used when a collapsed ref target is an abstract head and must be emitted under its concrete substitution-group member name (via `resolveConcreteElement`)
- `scripts/lib/codegens.ts` — TypeScript code generators. Each function takes `(netexLibrary, name, opts?)` and returns a string. The `opts.html` flag (default `true`) toggles HTML-highlighted vs plain-text output; `opts.excludeProps` (`Set<string>`) filters properties from generated output. `generateInterface` emits `/** @deprecated */` JSDoc for properties stamped with `x-netex-deprecated`. Exports: `generateInterface`, `generateTypeAlias`, `generateSubTypesBlock`, `collectRenderableDeps`, `toConstName`, `escHtml`
...
- `scripts/lib/loader.ts` — schema loader. Exports `loadNetexLibrary()` — loads the base assembly schema from `generated-src/base`. Extracted from `test-helpers.ts` so both tests and CLI scripts (`ts-gen.ts`) can share it
- `scripts/lib/__tests__/` — per-module unit tests (mock schemas) and integration tests (real generated schema). Shared `test-helpers.ts` provides `loadNetexLibrary()` (re-exported from `loader.ts`), XSD validation utilities (`requireXsd`, `validateWithXmllint`, `nonKeyrefErrors`), entity wrapping (`wrapInPublicationDelivery`), and shared fixtures (`CORE`, `EXTENSIVE`). `valid-roundtrip.test.ts` validates fake→XML output against NeTEx XSD via `xmllint`. `valid-generated-roundtrip.test.ts` validates generated mapping code produces valid XML (flattenFake → eval'd mapping → xmllint)
- `scripts/static/schema-viewer-host-app.js` — browser-side controller for the self-contained `netex-schema.html` page. Read by `build-schema-html.ts` and embedded verbatim inside a `<script>` tag after the `/*@@VIEWER_FNS@@*/` placeholder is replaced with the esbuild-bundled viewer-fns IIFE. Key internal structures: `TAB_MAP` (maps tab `data-tab` keys to panel element IDs — includes `relations` for the bipartite entity graph), `attachCopyHandler(container)` (shared copy-to-clipboard wiring for code blocks). Responsibilities: sidebar search and role-chip filtering, explorer panel lifecycle (open/close, tab switching via `TAB_MAP`, resize), HTML builders for explorer tabs (graph SVG diagram, relations bipartite graph with ref-property dropdown, interface with "+N more types" dependency expansion via `collectDependencyTree`, mapping converters, sample data with Flat/XmlShaped/XML three-pill toggle — all three pills react to property checkbox and hide-base-props toggles; Flat pill uses `flattenFake()`), "Used by entities" dropdown (BFS via `findTransitiveEntityUsers`), via-chain hover popup on interface properties. The TypeScript tab (`renderInterfaceHtml`) delegates to bundled `generateInterface` from `codegens.ts` — the host-app only adds DOM wrappers (copy buttons, section headers). Style rule: no inline `style=` attributes in generated HTML — use CSS classes/IDs from `schema-viewer.css`; runtime `.style.*` for computed geometry (resize, popups) is OK
...
- `xsd-to-jsonschema.js` — **primary** XSD → JSON Schema converter (invoked via Makefile). Uses `Java.type()` for DOM parsing (`DocumentBuilderFactory`, `org.w3c.dom.Node`). Plain JavaScript, no modules, no npm. The canonical implementation. Stamps ten `x-netex-*` annotations on definitions: `x-netex-source`, `x-netex-assembly`, `x-netex-role` (with `/deprecated` suffix for deprecated definitions), `x-netex-atom`, `x-netex-frames`, `x-netex-mixed`, `x-netex-substitutionGroup`, `x-netex-sg-members`, `x-netex-refTarget`, `x-netex-collapsed`. Additionally stamps two per-property annotations: `x-netex-choice` (choice group siblings) and `x-netex-deprecated` (boolean, for properties with uppercase DEPRECATED in description). Supports `--parts` (config keys or natural names), `--sub-graph <TypeName>` (prune to reachable definitions), and `--collapse` (collapse transparent wrappers in sub-graphs). See `json-schema/README.md` for full annotation documentation
...
Primary conversion path. Uses Java standard library DOM APIs via GraalVM interop. Stamps ten per-definition annotations: `x-netex-source` (origin XSD file), `x-netex-role` (entity, structure, reference, enumeration, abstract, collection, view, frameMember — with `/deprecated` suffix appended for definitions whose description contains uppercase `DEPRECATED`), `x-netex-atom` (atom type for transparent wrappers — primitive string for value-only simpleContent types, `"simpleObj"` for multi-prop simpleContent, `"array"` for `xsd:list` types), `x-netex-frames` (frame membership list), `x-netex-mixed` (mixed content flag), `x-netex-substitutionGroup` (head element name), `x-netex-sg-members` (member list on head elements), `x-netex-refTarget` (target element name for reference-role defs, e.g. `TransportTypeRef` → `"TransportType"`), and `x-netex-collapsed` (count, set by `--collapse` pass). Additionally stamps two per-property annotations: `x-netex-choice` (array of sibling property names from the same `xsd:choice` — used by the data faker to emit only the first alternative) and `x-netex-deprecated` (boolean `true` for properties with uppercase `DEPRECATED` in description — used by the schema viewer for strikethrough styling and by `generateInterface` for `@deprecated` JSDoc). The schema HTML viewer uses the per-definition annotations for role filtering, type resolution (stamped enumerations stop at the name rather than expanding to literal unions; stamped arrays resolve their items), and transitive entity usage lookups. Optional `--sub-graph <TypeName>` prunes the schema to definitions reachable from a root type; `--collapse` then inlines transparent wrappers.

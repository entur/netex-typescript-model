...
- `scripts/lib/data-faker.ts` — data generation functions extracted from fns.ts. Builds fully populated fake objects from JSON Schema definitions and serializes them to XML. Primary export: `fake` (aliased as `genMockObject` for backward compat). Also exports: `defaultForType` (default value factory for primitive types), `serializeValue` (canonical `$`-prefixed props to fast-xml-parser `@_`-prefixed shape), `buildXmlString` (fake → XML string via XMLBuilder), `toXmlShape` (schema-aware transform to XMLBuilder shape — property ordering per XSD sequence, `value→#text` for simpleContent, `$→@_` for attributes, recursive ref-target resolution; aliased as `toValidNested` for backward compat), `serialize` (wraps `toXmlShape` + `XMLBuilder` for single-call XML output). Internal helpers: `fakeRef` (builds ref-structure fakes), `tryFakeShallow` (single-pass shallow-complex fake builder — calls `flattenAllOf` once, bails with `null` if any prop is complex, otherwise returns the filled fake object)

...
- `scripts/lib/schema-nav.ts` — inheritance walking and property flattening. Exports: `flattenAllOf`, `collectRequired`, `buildInheritanceChain`, `inlineSingleRefs` (expands 1-to-1 `$ref` properties by splicing target's own props; skips collection-role, reference-role, and atom targets), `buildExclSet` (builds a `Set<string>` of property names to exclude, merging explicit exclusions with optional omnipresent filtering), `OMNIPRESENT_DEFS`

...
- `gen-vehicletype.sh` — convenience wrapper: generates VehicleType `.ts` files with hathor-specific `--exclude` list. Calls `ts-gen.ts` internally
- `gen-deckplan.sh` — convenience wrapper: generates DeckPlan `.ts` files with NeTEx-Deckplan-Editor `--exclude` list. Calls `ts-gen.ts` internally
...
- `scripts/lib/collapse.ts` — collapse resolution for `--collapse-refs` and `--collapse-collections`. Exports: `CollapseOpts`, `CollapsedRef`, `CollapsedColl`, `collapseRef` (ref property → `Ref<'Entity'>` / `SimpleRef`), `collapseColl` (single-child RelStructure → child entity type), `resolveCollVerStruct` (def-name variant for BFS), `buildTypeOverrides` (builds `Map<canonName, typeStr>` for `generateInterface`), `REF_PREAMBLE`

...
- `Makefile` — build orchestrator: XSD download → JSON Schema → schema HTML → TypeScript → TypeDoc → tarball. Parses `NETEX_VERSION`/`NETEX_BRANCH` from config. Key targets: `all`, `schema`, `types`, `docs`, `tarball`, `clean`. The schema HTML target depends on both the JSON Schema file and the embedded source files (`build-schema-html.ts`, `fns.ts`, `data-faker.ts`, `codegens.ts`, `bundle-entry.ts`, `schema-viewer-host-app.js`, `schema-viewer.css`) so edits to viewer logic trigger a rebuild
...
- `scripts/lib/fns.ts` — schema introspection functions shared between the schema HTML viewer and unit tests. **Keep this description in sync when editing the file.** Internal helpers: `deref` (strip `#/definitions/` prefix), `allOfRef` (first `$ref` in allOf). Exports: `classifySchema` (discriminated `SchemaShape` union — single-pass classification used by `resolveType`, `isRefType`, `refTarget`, `resolvePropertyType`), type introspection (`resolveType`, `isRefType`, `refTarget`), mixed-content detection (`unwrapMixed`), inheritance walking (`flattenAllOf`, `collectRequired`, `buildInheritanceChain`), deep type resolution (`resolveDefType`, `resolvePropertyType`, `resolveAtom`), interface inlining (`inlineSingleRefs` — expands 1-to-1 `$ref` properties by splicing the target's own props into the parent; skips collection-role, reference-role, and atom targets so transparent wrappers like `PrivateCodeStructure` stay as single properties), reverse index (`buildReverseIndex`, `findTransitiveEntityUsers` — takes an `isTarget` predicate), relations tab helpers (`resolveRefEntity` — resolves a reference def to its target entity/entities with cycle-safe abstract expansion via `x-netex-sg-members`; `collectRefProps` — walks `flattenAllOf` to find ref-typed properties with resolvable entity targets; `collectExtraProps` — finds properties an entity's structure adds beyond a base structure by walking the allOf chain), dependency tree (`collectDependencyTree` — BFS walker collecting all transitive type dependencies of a definition as `DepTreeNode[]` with name, via, depth, duplicate tracking; `resolveAlias` follows both `$ref` aliases and allOf-passthrough wrappers so transparent wrappers like `PrivateCode → PrivateCodeStructure` resolve to the underlying def; stops at enums, references, atoms, and non-complex types but still follows array-item refs and `anyOf` branches on leaf nodes to collect list-of-enum and union-enum targets), role filter logic (`defRole`, `countRoles`, `presentRoles`, `ROLE_DISPLAY_ORDER`, `ROLE_LABELS`), code-generation helpers (`lcFirst`, `canonicalPropName`). `resolveDefType` uses `x-netex-*` stamps to drive resolution: `x-netex-atom:"array"` resolves list items, `x-netex-role:"enumeration"` stops at the enum name, `x-netex-atom` collapses simpleContent wrappers; maps JSON Schema `integer` to TypeScript `number`; treats completely empty schemas (`{}`) as `any` (empty-object rule). Re-exports all data-faker functions for backward compatibility. Bundled via esbuild (through `bundle-entry.ts`) into an IIFE with `globalName: "_viewerBundle"` (including `fast-xml-parser`) and embedded in the HTML page via the `/*@@VIEWER_FNS@@*/` placeholder in `schema-viewer-host-app.js`. Bound wrappers close over the page-level `defs` variable so the host app can call functions without passing `defs` explicitly
- `scripts/lib/data-faker.ts` — data generation functions extracted from fns.ts. Builds fully populated fake objects from JSON Schema definitions and serializes them to XML. Primary export: `fake` (aliased as `genMockObject` for backward compat). Also exports: `defaultForType` (default value factory for primitive types), `buildXml` (thin XMLBuilder wrapper for pre-transformed objects — takes `toXmlShape` output and produces formatted XML), `toXmlShape` (schema-aware transform to XMLBuilder shape — property ordering per XSD sequence, `value→#text` for simpleContent, `$→@_` for attributes, recursive ref-target resolution), `serialize` (composes `toXmlShape` + `buildXml` for single-call XML output). Internal helpers: `fakeRef` (builds ref-structure fakes), `tryFakeShallow` (single-pass shallow-complex fake builder — calls `flattenAllOf` once, bails with `null` if any prop is complex, otherwise returns the filled fake object). `fake` respects `x-netex-choice` annotations — only the first alternative from each choice group is emitted
- `scripts/lib/codegens.ts` — TypeScript/utility code generators extracted from host-app. Each function takes `(defs, name, opts?)` and returns a string. The `opts.html` flag (default `true`) toggles between HTML-highlighted output (for the viewer) and plain-text output (testable, CLI-usable). Exports: `generateInterface` (flat interface from allOf chain — delegates to `generateTypeAlias` for empty-prop defs), `generateTypeAlias` (enum const-array pattern or simple `type X = Y`), `generateTypeGuard` (`isFoo(o)` with typeof/Array.isArray checks), `generateFactory` (`createFoo(init?)` with required-field defaults), `toConstName` (PascalCase → UPPER_SNAKE)
- `scripts/lib/bundle-entry.ts` — esbuild entry point that re-exports all public functions from `fns.ts`, `data-faker.ts`, and `codegens.ts`. esbuild bundles this into a single IIFE for embedding in the HTML page
